...
// }'
//
// 从环境变量或 localStorage 读取配置（优先级：环境变量 > localStorage）
// 注意：不再使用硬编码的默认值，必须通过环境变量或应用内配置
const getConfig = () => {
  ...
  try {
    ...
    return {
      ...
      workflowId: envWorkflowId || storedWorkflowId || '',
    };
  } catch (e) {
    console.warn('无法读取 localStorage，使用环境变量:', e);
    return {
      token: envToken || '',
